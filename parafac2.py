...
class parafac2:        
    ...
    def L2_loss_dense(self, is_train, batch_size, input_U):
        _loss = 0
        Vprod = self.V[0]
        for i in range(1, self.tensor.order-2):
    ...
    def L2_loss(self, is_train, batch_size, input_U):                
        ...
        for i in range(self.tensor.order-2):
    ...
    def L2_loss_tucker(self, batch_loss_zero, batch_loss_nz):        
        with torch.no_grad():
            ...
            for i in tqdm(range(0, self.tensor.num_nnz, batch_loss_nz)):
                ...
                # Prepare matrices
                curr_indices = [torch.tensor(self.tensor.indices[m][i:i+curr_batch_size], dtype=torch.long, device=self.device) for m in range(self.tensor.order)]                
    ...
    def als_U(self, args):    
        with torch.no_grad():                   
            ...
            for i in tqdm(range(0, self.tensor.num_tensor, args.tucker_batch_u)):
                curr_batch_size = min(args.tucker_batch_u, self.tensor.num_tensor - i)
                ...
                if args.is_dense:     
                    ...
                    last_idx = self.tensor.indices[-1][curr_tidx:next_tidx]
                    last_idx = torch.tensor(last_idx, dtype=torch.long, device=self.device)
                    first_idx = self.tensor.indices[0][curr_tidx:next_tidx]
                    ...
                    for m in range(self.tensor.order-2):
                        curr_idx = self.tensor.indices[m+1][curr_tidx:next_tidx]
                    ...
                    curr_values = self.tensor.values[curr_tidx:next_tidx]
                ...
                GVSG = torch.bmm(torch.transpose(curr_S.unsqueeze(1), 1, 2), curr_S.unsqueeze(1))    # batch size x rank x rank 
                GVSG = batch_kron(VtV.repeat(curr_batch_size, 1, 1), GVSG)   # batch size x rank^(d-1) x rank^(d-1)
                GVSG = torch.bmm(mat_G.repeat(curr_batch_size, 1, 1), GVSG)  # batch size x rank x rank^(d-1)
                GVSG = torch.bmm(GVSG, mat_G.t().repeat(curr_batch_size, 1, 1))  # batch size x rank x rank                                 
                GVSG = GVSG.unsqueeze(1).expand(-1, self.tensor.max_first, -1, -1)  # batch size x i_max x rank x rank
                GVSG = GVSG[curr_mapping_mask]   # small bs x rank x rank
                curr_mapping = curr_mapping[curr_mapping_mask]  # Small bs                
                second_mat = second_mat.index_add_(0, curr_mapping, GVSG)
    ...
    def als_V(self, args, mode):            
        with torch.no_grad():
            # Define mat_G
            perm_dims = [mode] + [m for m in range(self.tensor.order) if m != mode]
            ...
            second_mat = 0
            for i in tqdm(range(0, self.tensor.num_tensor, args.tucker_batch_v)):                
                # Build the first mat
                curr_batch_size = min(args.tucker_batch_v, self.tensor.num_tensor - i)        
                ...
                if args.is_dense:
                    ...
                    perm_dims = [0, mode+1] + [m for m in range(1, self.tensor.order-1) if m != mode+1]    
                    curr_tensor = torch.permute(curr_tensor, perm_dims)   # bs x i_m x i_max x i_2 x ... x i_(m-1)  
                    curr_tensor = torch.reshape(curr_tensor, (curr_batch_size, self.tensor.middle_dim[mode-1], -1))   # bs x i_m x i_max x i_2*...*i_(m-1)               
                                        
                    curr_mapping = self.mapping[i:i+curr_batch_size, :]   # batch size x i_max
                    UVS = curr_U  # batch size x i_max x R                
                    if self.tensor.order > 3:
                        UVS = batch_kron(UVS, Vkron.repeat(curr_batch_size, 1, 1))   # batch size x i_max*j_1*...*j_(d-2) x R^(d-2)
                    UVS = batch_kron(UVS, curr_S.unsqueeze(1))   # batch size x i_max*j_1*...*j_(d-2) x R^(d-1)                    
                    temp_fm = torch.bmm(curr_tensor, UVS)   # batch size x i_m x R^(d-1)
                    ...
                    first_idx = self.tensor.indices[0][curr_tidx:next_tidx]   # bs'
                    first_idx = torch.tensor(first_idx, dtype=torch.long, device=self.device)  # bs'
                    last_idx = self.tensor.indices[-1][curr_tidx:next_tidx]   # bs'
                    last_idx = torch.tensor(last_idx, dtype=torch.long, device=self.device)  # bs'
                    XUVS = curr_U[last_idx - i, first_idx, :]  # bs' x R
                    ...
                    for m in range(1, self.tensor.order - 1):
                        if m == mode: continue
                            ...
                            XUVS = face_split(XUVS, self.V[m-1][curr_idx, :])   
                                    
                    #print(XUVS.shape)
                    #print(self.V[-1][curr_idx,:].shape)
                    XUVS = face_split(XUVS, self.S[last_idx, :])   # bs' x R^(d-1)
                    
                    curr_values = self.tensor.values[curr_tidx:next_tidx]   # bs'
                    curr_values = torch.tensor(curr_values, dtype=torch.double, device=self.device)  # bs'
                    XUVS = curr_values.unsqueeze(-1) * XUVS  # bs' x R^(d-1)
                    temp_first_mat = XUVS @ mat_G.t()   # bs' x R
                    
                    curr_idx = self.tensor.indices[mode][curr_tidx:next_tidx]  # bs'
                    curr_idx = torch.tensor(curr_idx, dtype=torch.long, device=self.device)  # bs'
                    first_mat = first_mat.index_add_(0, curr_idx, temp_first_mat)  # j_m x R
                
                # Build second mat                  
                curr_sm = torch.bmm(torch.transpose(curr_U, 1, 2), curr_U)  # batch size x R x R
                if self.tensor.order > 3:
                    curr_sm = batch_kron(curr_sm, VtV.repeat(curr_batch_size, 1, 1))
                
                StS = torch.bmm(torch.transpose(curr_S.unsqueeze(1), 1, 2), curr_S.unsqueeze(1)) # batch size x R x R            
                curr_sm = batch_kron(curr_sm, StS)   # bathc size x R^(d-1) x R^(d-1)
                second_mat = second_mat + torch.sum(curr_sm, dim=0)   # R x R
    ...
    def als_S(self, args):        
        with torch.no_grad():
            ...
            for i in tqdm(range(0, self.tensor.num_tensor, args.tucker_batch_s)):
                curr_batch_size = min(args.tucker_batch_s, self.tensor.num_tensor - i)
                curr_mapping = self.mapping[i:i+curr_batch_size, :]   # batch size x i_max
                curr_U = self.centroids.data[curr_mapping] * self.U_mask[i:i+curr_batch_size, :, :]   # batch size x i_max x R
                # first mat
                if args.is_dense:
                    ...
                    first_mat = batch_kron(curr_U, Vkron.repeat(curr_batch_size, 1, 1))  # batch size x i_1*i_2*...i_m-1 x R^(d-1)
                    first_mat = torch.bmm(curr_tensor.unsqueeze(1), first_mat)   # batch size x 1 x R^(d-1)
                    first_mat = first_mat.squeeze() @ mat_G.t()  # batch size x R
                    if curr_batch_size == 1:
                        first_mat = first_mat.unsqueeze(0)
                    ...
                    XUVG = curr_U[last_idx-i, first_idx, :]   # bs' x rank
                    for m in range(1, self.tensor.order-1):
                        curr_idx = self.tensor.indices[m][curr_tidx:next_tidx]
                        curr_idx = torch.tensor(curr_idx, dtype=torch.long, device=self.device)
                    ...
                    first_mat = torch.zeros((curr_batch_size, self.rank), dtype=torch.double, device=self.device)
                    first_mat = first_mat.index_add(0, last_idx-i, XUVG)  # bs x rank
                    
                # end mat
                second_mat = torch.bmm(torch.transpose(curr_U, 1, 2), curr_U)   # batch size x R x R
                second_mat = batch_kron(second_mat, VtV.repeat(curr_batch_size, 1, 1))    # batch size x R^(d-1) X R^(d-1)
                second_mat = torch.bmm(mat_G.repeat(curr_batch_size, 1, 1), second_mat)   # batch size x R x R^(d-1)
                second_mat = torch.bmm(second_mat, mat_G.t().repeat(curr_batch_size, 1, 1))   # batch size x R x R
                second_mat = torch.linalg.pinv(second_mat)     # batch size x R x R
                
                self.S.data[i:i+curr_batch_size, :] = torch.bmm(first_mat.unsqueeze(1), second_mat).squeeze()   
        ...
        with torch.no_grad():                             
            ...
            if args.is_dense:
                ...
                for m in range(self.tensor.order-2):
                    if m == 0:
                        ...
                        Vkron = batch_kron(Vkron, self.V[m].data.unsqueeze(0))
                Vkron = Vkron.squeeze()   # i_2*...*i_(m-1) x rank^(d-2)
            ...
            for i in tqdm(range(0, self.tensor.num_tensor, args.tucker_batch_g)):
                ...
                if args.is_dense:
                    # Build tensor
                    curr_tensor = self.tensor.src_tensor_torch[i:i+curr_batch_size].to(self.device)  # bs x i_max x i_2 x ... x i_(m-1)                      
                    curr_tensor = torch.reshape(curr_tensor, (curr_batch_size, -1))  # bs x i_1*... x i_(m-1)
                    curr_S = self.S.data[i:i+curr_batch_size, :]   # bs x rank
                    USV = batch_kron(curr_U, curr_S.unsqueeze(1))    # bs x i_max x rank^2
                    USV = batch_kron(USV, Vkron.repeat(curr_batch_size, 1, 1))  # bs x i_1*..*i_(m-1) x rank^d
                    temp_sm = torch.bmm(torch.transpose(USV, 1, 2), curr_tensor.unsqueeze(-1)) # bs x rank^d x 1
                    second_mat = second_mat + torch.sum(temp_sm.squeeze(), dim=0)   #  rank^d
                else: 
                    curr_tidx = self.tensor.tidx2start[i]
                    next_tidx = self.tensor.tidx2start[i + curr_batch_size]
                    ...
                    USVX = curr_U.data[curr_li - i, curr_fi, :]   # bs' x rank
                    USVX = face_split(USVX, self.S[curr_li, :].data)
                    for m in range(1, self.tensor.order-1):
                        curr_idx = self.tensor.indices[m][curr_tidx:next_tidx]
                        curr_idx = torch.tensor(curr_idx, dtype=torch.long, device=self.device)
                        _V = self.V[m-1][curr_idx, :].data     # bs' x rank
                        USVX = face_split(USVX, _V)
                    # USVX: bs' x rank^d 
                                        
                    curr_values = torch.tensor(self.tensor.values[curr_tidx:next_tidx], dtype=torch.float, device=self.device)   # bs'
                    USVX = USVX * curr_values.unsqueeze(-1)  # bs' x rank^d
                    temp_second_mat = torch.sum(USVX, dim=0)  # rank^d
                    second_mat = second_mat + temp_second_mat  # rank^d
            
            second_mat = second_mat.unsqueeze(-1)   # rank^d x 1
            ...
            first_mat = batch_kron(US.unsqueeze(0), VtV.unsqueeze(0)).squeeze()    # rank^d x rank^d
            first_mat = torch.linalg.pinv(first_mat)                            
            self.G.data = (first_mat @ second_mat).squeeze()   # R^d
