...
class parafac2:
    ...
    def __init__(self, _tensor, device, args):      
        ...
        self.Q_mask = torch.zeros((_tensor.k, _tensor.i_max, self.rank), device=device, dtype=torch.double)   # k x i_max x rank, 1 means the valid area
        ...
        self.Q, self.H = torch.nn.Parameter(self.Q), torch.nn.Parameter(self.H)
        self.S, self.V = torch.nn.Parameter(self.S), torch.nn.Parameter(self.V)        
        with torch.no_grad():
            print(f'fitness: {1 - math.sqrt(self.L2_loss(args.batch_size, self.Q))/math.sqrt(self.tensor.sq_sum)}') 
            print(f'square loss: {self.L2_loss(args.batch_size, self.Q)}')
        
        
                    
    def L2_loss(self, batch_size, input_Q):        
        # V * sigma * H^T
        ...
        temp_tensor = torch.matmul(input_Q, temp_tensor)    # k x i_max x rank
        sq_sum = torch.sum(temp_tensor * input_Q)
        ...
        for i in range(0, self.tensor.num_nnz, batch_size):
            ...
            _Q = input_Q[_height, _row, :]  # batch size x rank
    ...
    def clustering(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.tensor.i_max):
                curr_dist = self.Q - self.centroids[i,:].unsqueeze(0).unsqueeze(0) # k x i_max x rank
    ...
    def quantization(self, args):
        optimizer = torch.optim.Adam([self.Q, self.H, self.S, self.V, self.centroids], lr=args.lr)
        
        for _epoch in tqdm(range(args.epoch)):
            optimizer.zero_grad()
            # Clustering     
            Q_clustered = self.centroids[self.clustering()]    # k x i_max x rank
            Q_clustered = Q_clustered * self.Q_mask
            sg_part = (self.Q - Q_clustered).detach()
            Q_tricked = self.Q - sg_part # refer to paper   # k x i_max x rank
            _loss = self.L2_loss(args.batch_size, Q_tricked) + torch.sum(torch.square(Q_clustered - self.Q.detach()))
            _loss.backward()
            optimizer.step()
            ...
            if (_epoch + 1) % 10 == 0:
                with torch.no_grad():
                    Q_clustered = self.centroids[self.clustering()]    # k x i_max x rank
                    Q_clustered = Q_clustered * self.Q_mask
                    _loss = self.L2_loss(args.batch_size, Q_clustered)
